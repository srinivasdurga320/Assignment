    ...
    reader.onload = (e) => {
      ...
      const trimmedData = parsedData.map(entry => {
        ...
        Object.entries(entry).forEach(([key, value]) => {
          ...
          const trimmedValue = typeof value === 'string' ? String(value).trim() : String(value);
    ...
    <div className="homepage-container">
      ...
      >
        ...
        <label htmlFor="fileInput" className="upload-label">
          <FaCloudUploadAlt size={50} color={dragging ? '#007bff' : '#999999'} />
          {dragging ? (
            <p>Drop the files here ...</p>
          ) : (
            <h4>{fileName ? fileName : 'Drag & drop file here or click to select files'}</h4>
          )}
          <input
            id="fileInput"
            type="file"
        ...
        <div>
          ...
          <button className="upload-button" onClick={cancel} disabled={files.length === 0}>
            Cancel
          </button>
        </div>
